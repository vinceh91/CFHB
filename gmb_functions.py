...
def get_locations(service: googleapiclient.discovery.Resource, accounts_list: list) -> pd.DataFrame:
    df_loc = pd.DataFrame()
...
def get_reviews(service: googleapiclient.discovery.Resource, df_loc: pd.DataFrame, date_ref: str) -> pd.DataFrame:
    all_reviews = []
    for loc in df_loc['name']:
        ...
        while True:
            if page_token:
                params['pageToken'] = page_token
            current_page = service.accounts().locations().reviews().list(**params).execute()
            page_token = current_page.get('nextPageToken')
...
def write_to_blob(blob_secret: str, container: str, blob: str, df: pd.DataFrame) -> None:
    blobstock_service_client = BlobServiceClient.from_connection_string(blob_secret)
    blob_client = blobstock_service_client.get_blob_client(container=container, blob=blob)
